export const TEST_DATA_WITH_SAFE_ALIAS = [
  ...
  [
    {
      testName: 'Not In',
      expectedSQL: `SELECT orders.* FROM (SELECT customer_id AS orders__customer_id, vendors AS orders__vendors, * FROM (select * from orders) AS orders) AS orders WHERE (((orders__customer_id NOT IN ('1', '2')) OR (orders__customer_id IS NULL)) AND ((NOT (orders__vendors && (ARRAY['myntra', 'flipkart']))) OR (orders__vendors IS NULL)))`,
      cubeInput: {
        measures: ['*'],
        filters: [
          {
