...
describe('Cube Schema to SQL', () => {
  ...
  describe('useDotNotation: true', () => {
    it('Should be able to execute sql query from cube string', async () => {
      ...
      console.info('parsedOutput', output);
      expect(sql).toEqual(
        'SELECT COUNT(*) AS "products.count" ,   "suppliers.created_at",  "products.created_at" FROM (SELECT products.created_at AS "products.created_at", * FROM (select * from products) AS products LEFT JOIN (SELECT suppliers.created_at AS "suppliers.created_at", * FROM (select * from suppliers) AS suppliers) AS suppliers  ON products.supplier_id = suppliers.id) AS MEERKAT_GENERATED_TABLE GROUP BY "suppliers.created_at", "products.created_at"'
      );
      expect(output).toHaveLength(3);
    });
  });
