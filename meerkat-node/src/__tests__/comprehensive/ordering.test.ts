...
describe('Comprehensive: Ordering', () => {
  ...
  describe('Single Column Ordering - Numeric', () => {
    it('should order BIGINT ascending', async () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].id_bigint)).toBeGreaterThan(
          Number(result[i - 1].id_bigint)
        );
      }
    });
    it('should order BIGINT descending', async () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].id_bigint)).toBeLessThan(
          Number(result[i - 1].id_bigint)
        );
      }
    });
    it('should order DOUBLE ascending', async () => {
      ...
      // Verify ascending order
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].metric_double)).toBeGreaterThanOrEqual(
          Number(result[i - 1].metric_double)
        );
      }
    });
    it('should order DOUBLE descending', async () => {
      ...
      // Verify descending order
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].metric_double)).toBeLessThanOrEqual(
          Number(result[i - 1].metric_double)
        );
      }
    });
  ...
  describe('Single Column Ordering - Dates', () => {
    it('should order DATE ascending', async () => {
      ...
      const firstDate =
        result[0].record_date instanceof Date
      ...
      for (let i = 1; i < result.length; i++) {
        const curr =
          result[i].record_date instanceof Date
            ? result[i].record_date
            : new Date(result[i].record_date);
        const prev =
          result[i - 1].record_date instanceof Date
            ? result[i - 1].record_date
            : new Date(result[i - 1].record_date);
    ...
    it('should order by different date fields', async () => {
      ...
      for (let i = 1; i < result.length; i++) {
        const prevCreated =
          result[i - 1].created_date instanceof Date
            ? result[i - 1].created_date
            : new Date(result[i - 1].created_date);
        const currCreated =
          result[i].created_date instanceof Date
            ? result[i].created_date
            : new Date(result[i].created_date);
  ...
  describe('Ordering with Aggregates', () => {
    ...
    it('should order grouped results by SUM', async () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].total)).toBeLessThanOrEqual(
    ...
    it('should order grouped results by AVG', async () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(Number(result[i].average)).toBeGreaterThanOrEqual(
          Number(result[i - 1].average)
        );
      }
    });
