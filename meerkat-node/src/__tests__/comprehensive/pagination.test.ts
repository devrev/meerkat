...
describe('Comprehensive: Pagination (LIMIT / OFFSET)', () => {
  ...
  describe('LIMIT with OFFSET', () => {
    it.fails('should skip rows with OFFSET', async () => {
      const queryPage1 = {
        measures: ['fact_all_types.count'],
        dimensions: ['fact_all_types.priority'],
    ...
    it.fails(
      'should implement pagination correctly across multiple pages',
      async () => {
        const pageSize = 1;
        const pages: any[][] = [];
        ...
        const priorities = pages.map(
          (page) => page[0]?.fact_all_types__priority
        );
  ...
  describe('Pagination with Ordering', () => {
    it.fails('should maintain order with LIMIT', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        dimensions: ['fact_all_types.priority'],
    ...
    it.fails('should maintain order with LIMIT and OFFSET', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        dimensions: ['fact_all_types.status'],
      ...
      for (let i = 1; i < result.length; i++) {
        expect(
          result[i].fact_all_types__status >=
            result[i - 1].fact_all_types__status
        ).toBe(true);
      }
    });
  });
  describe('Pagination with Filters', () => {
    ...
    it.fails('should apply OFFSET after filtering', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        dimensions: ['fact_all_types.priority'],
        filters: [
  ...
  describe('Edge Cases', () => {
    ...
    it.fails('should handle LIMIT 0', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        dimensions: ['fact_all_types.priority'],
        limit: 0,
    ...
    it.fails('should handle OFFSET 0 (same as no offset)', async () => {
  ...
  describe('Performance', () => {
    ...
    it.fails('should execute large OFFSET query efficiently', async () => {
      const start = Date.now();
      const query = {
        measures: [],
