...
describe('Comprehensive: JSON Fields', () => {
  ...
  describe('JSON Extraction', () => {
    ...
    it.fails('should extract JSON field and cast to numeric', async () => {
      const sql = `
        SELECT 
  ...
  describe('JSON Filtering', () => {
    ...
    it.fails('should filter by JSON numeric field', async () => {
      const sql = `
        SELECT 
          COUNT(*) as count
        FROM fact_all_types
  ...
  describe('JSON with Aggregates', () => {
    ...
    it.fails('should use JSON field in complex aggregation', async () => {
      const sql = `
        SELECT 
          JSON_EXTRACT_STRING(metadata_json, '$.source') as source,
          priority,
