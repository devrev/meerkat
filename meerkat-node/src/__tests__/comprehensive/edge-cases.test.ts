...
describe('Comprehensive: Edge Cases & NULL Handling', () => {
  ...
  describe('NULL Handling', () => {
    it.fails('should handle NULL in numeric fields', async () => {
      const query = {
    ...
    it.fails('should handle NOT NULL in numeric fields', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        filters: [
          {
    ...
    it.fails('should handle NULL in date fields', async () => {
      const query = {
        measures: ['fact_all_types.count'],
        filters: [
          {
    ...
    it.fails(
      'should handle mixed NULL and non-NULL in aggregates',
      async () => {
        const sql = `
        SELECT 
          COUNT(*) as total,
  ...
  describe('Special Characters in Strings', () => {
    ...
    it.fails('should handle percent signs in LIKE patterns', async () => {
      const sql = `
        SELECT COUNT(*) as count
        FROM fact_all_types
    ...
    it.fails('should handle underscore in LIKE patterns', async () => {
      const sql = `
        SELECT COUNT(*) as count
        FROM fact_all_types
  ...
  describe('Extreme Values', () => {
    ...
    it.fails('should handle very small DOUBLE values', async () => {
      const sql = `
        SELECT 
      ...
      expect(Number(result[0].max_metric)).toBeGreaterThan(
        Number(result[0].min_metric)
      );
    });
  });
  describe('Empty Arrays', () => {
    it.fails('should count rows with empty tag arrays', async () => {
      const sql = `
        SELECT COUNT(*) as count
        FROM fact_all_types
