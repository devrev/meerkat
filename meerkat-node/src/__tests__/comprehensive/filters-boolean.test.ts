...
describe('Comprehensive: Boolean Filters', () => {
  ...
  describe('Complex Boolean Combinations', () => {
    it.fails(
      'should handle AND combination: is_active=true AND is_deleted=false',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
    ...
    it.fails(
      'should handle boolean with numeric filter: is_active=true AND id_bigint < 100000',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
    ...
    it.fails(
      'should handle boolean with string filter: is_active=true AND priority=high',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
  ...
  describe('Boolean Performance Tests', () => {
    it.fails(
      'should filter boolean fields quickly (< 500ms on 1M rows)',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
  ...
  describe('Boolean Edge Cases', () => {
    it.fails(
      'should handle contradictory boolean filters: is_active=true AND is_active=false',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
    ...
    it.fails(
      'should handle multiple boolean fields with different values',
      async () => {
        const query = {
          measures: ['fact_all_types.count'],
          filters: [
