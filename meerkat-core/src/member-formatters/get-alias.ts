 ...
 * ```
 */
export const getAliasForSQL = (name: string, tableSchema: TableSchema): string => {
  const [, field] = splitIntoDataSourceAndFields(name);
  return constructAliasForSQL(name, findInSchema(field, tableSchema)?.alias);
};
 ...
 * ```
 */
export const getAliasForAST = (name: string, tableSchema: TableSchema): string => {
  const [, field] = splitIntoDataSourceAndFields(name);
  return constructAliasForAST(name, findInSchema(field, tableSchema)?.alias);
};
