...
describe('cubeToDuckdbAST (useDotNotation: true)', () => {
  ...
  it('should handle complex query with dot notation', () => {
    const query: Query = {
      measures: ['test_table.measure1'],
      dimensions: ['test_table.dimension1'],
      filters: [
  ...
  it('should handle dimensions with multiple dots - preserves dot notation', () => {
    const complexTableSchema: TableSchema = {
      name: 'test_table',
      sql: 'test_table',
      measures: [
    ...
    expect(result.node.type).toBe(QueryNodeType.SELECT_NODE);
    expect(result.node.group_expressions).toHaveLength(1);
    // With dot notation, dots are preserved instead of converted to underscores
    expect(result.node.group_expressions[0]).toEqual({
      alias: '',
      class: 'COLUMN_REF',
  ...
  it('should handle aliases with dot notation - alias takes precedence', () => {
    const tableSchema: TableSchema = {
      name: 'test_table',
      sql: 'test_table',
      measures: [
    ...
    expect(result.node.type).toBe(QueryNodeType.SELECT_NODE);
    expect(result.node.group_expressions).toHaveLength(1);
    // Custom alias should still be used, regardless of useDotNotation
    expect(result.node.group_expressions[0]).toEqual({
      alias: '',
      class: 'COLUMN_REF',
      column_names: ['dimension_with_alias'],
