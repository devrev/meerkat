...
const defaultOptions = { filterType: 'PROJECTION_FILTER' as const, config: { useDotNotation: false } };
const dotNotationOptions = { filterType: 'PROJECTION_FILTER' as const, config: { useDotNotation: true } };

describe('cubeToDuckdbAST (useDotNotation: false)', () => {
  ...
  it('should return null if table schema is null', () => {
    const result = cubeToDuckdbAST({} as Query, null as unknown as TableSchema, defaultOptions);
...
describe('cubeToDuckdbAST (useDotNotation: true)', () => {
  const mockTableSchema: TableSchema = {
    name: 'test_table',
    sql: 'test_table',
    measures: [
  ...
  it('should return null if table schema is null', () => {
    const result = cubeToDuckdbAST({} as Query, null as unknown as TableSchema, dotNotationOptions);
    expect(result).toBeNull();
  });

  it('should handle basic query with dot notation', () => {
    const query: Query = {
      measures: ['test_table.measure1'],
      dimensions: ['test_table.dimension1'],
    };

    const result = cubeToDuckdbAST(query, mockTableSchema, dotNotationOptions);
    expect(result).not.toBeNull();
    expect(result.node.type).toBe(QueryNodeType.SELECT_NODE);
    expect(result.node.group_expressions).toHaveLength(1);
    expect(result.node.group_expressions).toEqual([
  ...
  it('should handle filters for dimensions (WHERE clause) with dot notation', () => {
    const query: Query = {
      measures: ['test_table.measure1'],
      dimensions: ['test_table.dimension1'],
      filters: [
  ...
  it('should handle filters for measures (HAVING clause) with dot notation', () => {
    const query: Query = {
      measures: ['test_table.measure1'],
      dimensions: ['test_table.dimension1'],
      filters: [
  ...
  it('should handle order by clause with dot notation', () => {
    const query: Query = {
      measures: ['test_table.measure1'],
      dimensions: ['test_table.dimension1'],
      order: {
  ...
  it('should handle dimensions with multiple dots in their names with dot notation', () => {
    const complexTableSchema: TableSchema = {
      name: 'test_table',
      sql: 'test_table',
      measures: [
    ...
    const result = cubeToDuckdbAST(query, complexTableSchema, dotNotationOptions);
    expect(result.node.type).toBe(QueryNodeType.SELECT_NODE);
    expect(result.node.group_expressions).toHaveLength(1);
    expect(result.node.group_expressions[0]).toEqual({
      alias: '',
      class: 'COLUMN_REF',
      column_names: ['test_table.dimension.with.dots'],
      type: 'COLUMN_REF',
    });
  });

  it('should handle aliases with dot notation', () => {
    const tableSchema: TableSchema = {
      name: 'test_table',
      sql: 'test_table',
      measures: [
    ...
    const result = cubeToDuckdbAST(query, tableSchema, dotNotationOptions);
    expect(result.node.type).toBe(QueryNodeType.SELECT_NODE);
    expect(result.node.group_expressions).toHaveLength(1);
    expect(result.node.group_expressions[0]).toEqual({
      alias: '',
